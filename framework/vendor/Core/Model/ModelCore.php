...
use Core\Model\EntityCore;

class ModelCore extends ModelDriver {
	 ...
	 * @link
	 */
	public $active_record_name = "Core\\EntityCore";
    /**
	 * モデルのインスタンスキャッシュ
	 * @var array 
	 * @link http://
    ...
    public function createEntity ($data) {
		$entity = $this->active_record_name;
	...
	public function fetchall() {
		...
		foreach($data as $key => $row) {
			$data[$key] = $this->createEntity($row);
		}
		return $data;
	}
	...
	public function new_record(){
		...
		unset($record[$key]);
		$record = $this->createEntity($record);
		return $record;
	}
